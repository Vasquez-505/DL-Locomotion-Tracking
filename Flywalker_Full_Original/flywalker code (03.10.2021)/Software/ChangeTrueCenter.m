function handles = ChangeTrueCenter(handles)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Check if TC has already been created - OUTDATED
% if isfield(handles, 'TC') == 0
%     handles.TC.coord = []; % new coordinates
%     handles.TC.frame = []; % frames in which the user changed the center
%     handles.TC.old = []; % coordinates of the previous center
%     handles.TC.vector = []; % saves information to plot bodytracks 
% end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Alexandre
% place only if x&y belong to the ellipse's axis
% get start and end points of body line
if handles.v.CurrentBodyDirection3(FrameNumber) == 1
    vector_x = handles.v.CurrentBodyX(FrameNumber) + [-1 1 ]*handles.v.CurrentBodyStdY(FrameNumber);
    vector_y = polyval([handles.v.CurrentBodyDirection1(FrameNumber) handles.v.CurrentBodyDirection2(FrameNumber)], vector_x);
else
    vector_y = handles.v.CurrentBodyY(FrameNumber) + [-1 1 ]*handles.v.CurrentBodyStdX(FrameNumber)+handles.p.cut.up-1;
    vector_x = polyval([handles.v.CurrentBodyDirection1(FrameNumber) handles.v.CurrentBodyDirection2(FrameNumber)], vector_y)+handles.p.cut.left-1;
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Alexandre
% vector of points from the body line
[xx,yy] = fillline([vector_x(1,1) vector_y(1,1)],[vector_x(1,2) vector_y(1,2)],100);

% round(xx,1)
% round(x,1)
% ismember(round(x,1),round(xx,1))
% 
% % round(yy,1)
% % round(y,1)
% ismember(round(y,1),round(yy,1))
...
% figure
% plot(vector_x+handles.p.cut.left-1,vector_y+handles.p.cut.up-1)
% figure
% plot(vector_x2+handles.p.cut.left-1,vector_y2+handles.p.cut.up-1)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Alexandre -> | ismember(...)==0 | ismember(...) == 0
% executes only if the coordinates are withing the body line

if x <= 0 | x >= handles.v.picsize(2) | y <= 0 | y >= handles.v.picsize(1) | ismember(round(x),round(xx))==0 | ismember(round(y),round(yy))==0
...
%     % save information regarding the previous center
%     handles.TC.old = [handles.TC.old; handles.v.CurrentBodyX(FrameNumber), handles.v.CurrentBodyY(FrameNumber)];
%     
%     % update information regarding frame number
%     handles.TC.frame = [handles.TC.frame;FrameNumber];
%     
%     % set data to specified, updating handles.TC.coord
%     handles.TC.coord = [handles.TC.coord; x - handles.p.cut.left + 1, y - handles.p.cut.up   + 1];
%     
%     % plot center with new coordinates
%     plot(handles.TC.coord(size(handles.TC.coord,1),1)+handles.p.cut.left-1,handles.TC.coord(size(handles.TC.coord,1),2)+handles.p.cut.up-1,'co', 'MarkerSize', handles.p.circlesize);
