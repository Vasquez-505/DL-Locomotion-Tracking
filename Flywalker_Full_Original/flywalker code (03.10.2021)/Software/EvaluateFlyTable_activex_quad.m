function FinishState = EvaluateFlyTable_activex_quad(filename, handles)
tic
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Added 27/03/2017
% generate an excel file (1-write excel file; 0-no excel file)
  ExcelOn = 1;
...
for i = 1:counter(1)
    % find middle leg and back leg that will considered the parts of the same 
...
%     plot(cos(t),sin(t),'color',[0.5,0.5,0.5],'LineWidth',2)
    ...
    title('Step AEPs');
    s5xpl = xlabel('perpendicular distance from body center [normalized to body length]');
    s5ypl = ylabel('parallel distance from body center [normalized to body length]');
    
    set(s5xpl, 'FontSize', 8); 
    set(s5ypl, 'FontSize', 9);
...
% PEPS    
    ...
    stc_trace_cluster_mean_all= (stc_traces_cluster(1)+stc_traces_cluster(3)+stc_traces_cluster(4)+stc_traces_cluster(6))/4;
...
% Calculate the distance difference between the AEP and PEP of left-right leg pairs
  ...
  PEPHindDiff   = sqrt(( StopPositionXAvg(3) -  StopPositionXAvg(6))^2 + ( StopPositionYAvg(3) -  StopPositionYAvg(6))^2);
    ...
    set(gca,'Layer','Top'); % put grid on top
    title('Step AEPs');
    s4pxl = xlabel('perpendicular distance from body center [normalized to body length]');
    s4pyl =  ylabel('parallel distance from body center [normalized to body length]') ;
    
    set(s4pxl, 'FontSize', 8); 
    set(s4pyl, 'FontSize', 9);
...
if PLOT == 1
    ...
    %                 text(AllLegsX(j,i) + 3, AllLegsY(j,i),TEXT(j,:),'Interpreter','none', 'Color',Color,'FontSize', 8);
            ...
            elseif length(X) == 4
                ...
                disp('You know nothing, Alexandre (line 1176, quad)')
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             plot(X+p.cut.left-1,Y+p.cut.up-1+50*(Common-1)/p.distcal,'Color',Color,'LineWidth', 1.5);
...
%             end;
            ...
            angle = atan(BodyDirection1(i));
            % Multiply by 4 to make sure the axis intersects the polygon
            FrontX = BodyX(i) + 4*BodyStdX(i)*sin(angle);
            FrontY = BodyY(i) + 4*BodyStdX(i)*cos(angle);
            X_axis = [FrontX 2*BodyX(i) - FrontX];
            Y_axis = [FrontY 2*BodyY(i) - FrontY];
%             plot(X+p.cut.left-1,Y+p.cut.up-1+50*(Common-1)/p.distcal,'Color',Color);
            [~,~,ii] = polyxpoly(X_axis+p.cut.left-1,Y_axis+p.cut.up-1+50*(Common-1)/p.distcal,poly_X,poly_Y);
%             if Orientation(i) == 1
%               plot(X(1)+p.cut.left-1,Y(1)+p.cut.up-1+50*(Common-1)/p.distcal,'yd','MarkerSize', 5, 'MarkerFaceColor','y');
%             elseif Orientation(i) == -1            
...
%             [d_TC,closest_x,closest_y] = p_poly_dist(TCx(i),TCy(i)+50*(Common-1)/p.distcal,[X X(1)],[Y Y(1)]+50*(Common-1)/p.distcal);
            ...
            if isempty(ii)
                ...
                if size(ii,1)>2
                    ...
                    disp('Alexandre, check line 1255 hexa. There are more than 2 intersections.')
                    return;
                else
                    ...
                    elseif ii(1,2) == 4
                        ...
                        disp('Alexandre, check line 1278 quad.')
                    end
                    ...
                    elseif ii(2,2) == 4
                        ...
                        disp('Alexandre, check line 1295 quad.')
                    end
                end;
...
%               disp(' ')
%               disp('CombinationCodes here')
%               disp(' ')
%               disp(CombinationCode)
%               disp(' ')
...
%new stance linearity is the ratio of stance displacement to pathlength
    ...
    for i=1:6
        ...
        for j = 1:(counter(i))
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Alexandre -> added if/else to avoid dividing by 0
            if Displacement(i,j)~=0 && pathlength(i,j)~=0
                StanceStraightness(i,j)=Displacement(i,j)/pathlength(i,j);
            else
                StanceStraightness(i,j)=0;
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end;
    end;
...
%   box on;
  ...
  % Things have to be between 0 and 1 (both x and y coordinates)
    ...
    if FourFeetOn == 0
      ...
      text(0,1-2*LineWidth,['Speed [mm/s]:           ' num2str(   mean(BodyVelocity(BodyVelocity > 0)/1000)     )],'FontSize', 10,'FontName','FixedWidth');
      text(0,1-3*LineWidth,['Pace index:             ' num2str(   PaceIndex/sum(Total(2:end))                 )],'FontSize', 10,'FontName','FixedWidth');
      text(0,1-4*LineWidth,['Trot index:             ' num2str(   TrotIndex/sum(Total(2:end))               )],'FontSize', 10,'FontName','FixedWidth');
      %text(0,1-4*LineWidth,['Body size:             ' num2str(   2*median(v.bodystdPar(:))                     )],'FontSize', 10,'FontName','FixedWidth');
      text(0,1-5*LineWidth,['Walk index:             ' num2str(   WalkIndex/sum(Total(2:end))               )],'FontSize', 10,'FontName','FixedWidth');
      text(0,1-6*LineWidth,['Non-canonical index:    ' num2str(   1- (PaceIndex + TrotIndex + WalkIndex)/sum(Total(2:end))                                    )],'FontSize', 10,'FontName','FixedWidth');
      ind = find(filename == '\' | filename == '/');
      text(0,1-7*LineWidth,['Stance straightness:    ' num2str(   mean(AvgStStrAll)                             )],'FontSize', 10,'FontName','FixedWidth');
            ind = find(filename == '\' | filename == '/');
      text(0,1-8*LineWidth,['Data name: ' num2str(   filename(ind(end-2)+1:ind(end-1)-1)                      )],'FontSize', 10,'FontName','FixedWidth', 'Interpreter', 'None');
    ...
    %text(0.55,1-7*LineWidth,['Average Step Dist:  ' num2str(   AvgStepDist                                   )],'FontSize', 10,'FontName','FixedWidth');
...
% exlSheet = exlFile.Sheets.Item('20.Triangles');
nbSheets = 20;
...
% write data
Range = strcat('A1:A',num2str(size(Data,1)));
ActivesheetRange = get(sheet,'Range',Range);
set(ActivesheetRange, 'Value', Data);
...
% 'TCx' 'TCy'
  ...
  % add header
    Data(1,1:end+2) = {'time' 'BodyX' 'BodyY' 'BodyDirection1' 'BodyDirection2' 'BodyDirection3' 'BodyOrientation' 'BodyStdX' 'BodyStdY' 'LeftFrontLegX' 'LeftFrontLegY' 'RightFrontLegX' 'RightFrontLegY' 'LeftMiddleLegX' 'LeftMiddleLegY' 'RightMiddleLegX' 'RightMiddleLegY' 'LeftBackLegX' 'LeftBackLegY' 'RightBackLegX' 'RightBackLegY' 'TCx' 'TCy' 'LegCombinations (LF LM LH RF RM RH)' 'Combination Code'};

  % add leg combinations to here
    for i = 1:length(LegCombinationArray)
        Data(i+1,end-1) = mat2cell(str2mat(dec2bin(LegCombinationArray(i),6)),1);
    end;

  % add CombinationCode
    Data(2:end,end) = num2cell(CombinationCode');
...
% write data
Range = strcat('A1:Y',num2str(size(Data,1)));
ActivesheetRange = get(sheet,'Range',Range);
set(ActivesheetRange, 'Value', Data);    
...
Range = 'A40:EA40';
ActivesheetRange = get(sheet,'Range',Range);
set(ActivesheetRange, 'Value', {'label','speed','pace index','trot index','trip/tetra','freq','period','avr FP STD','avr perp M','sw v(AVR)','sw v(SD)','sw v(F)','sw v(M)','sw v(H)','sw s(AVR)','sw s(SD)','sw s(F)','sw s(M)','sw s(H)','sw t(AVR)','sw t(SD)','sw t(F)','sw t(M)','sw t(H)','period SD','stc t(F)','stc t(M)','stc t(H)','stc t(AVR)','stc t(SD)','varAEP_F','varAEP_M','varAEP_H','varPEP_F','varPEP_M','varPEP_H','stnc stabl','SD AEP','SD PEP','body size','F AEP','M AEP','H AEP','F PEP','M PEP','H PEP','F AEPy','M AEPy','H AEPy','F PEPy','M PEPy','H PEPy','F AEPySD','M AEPySD','H AEPySD','F PEPySD','M PEPySD','H PEPySD','F AEPx','M AEPx','H AEPx','F PEPx','M PEPx','H PEPx','F AEPxSD','M AEPxSD','H AEPxSD','F PEPxSD','M PEPxSD','H PEPxSD','PeriodSD1000','Speed SD','4feetON','framesON','body stbl','2+ pfON','F_AEP STD','M_AEP_AEP STD','H_AEP STD','F_PEP STD','M_PEP STD','H_PEP STD','(empty)','SwingV STD','F_deltaEP','M_deltaEP','H_deltaEP','non-can.','AVRcombTrace','STDcombTrace','AVR perpM','Max Mdist','swT+stT','duty Fact','Phase F','Phase M','Phase H','Phase LH vs LM','Phase LM vs LF','Phase RH vs RM','Phase RM vs RF','walk indexBODA','tTripod','tTransition','Stc Str F','Stc Str M','Stc Str H','Overall Stc Str','Overall Avg Disp','Overall Avg Path','Body displacement (um)', 'Total body path (um)', 'Body displacement ratio', 'Stability index', 'Average onset ratio', 'Average offset ratio', 'Average stability ratio', 'Average area of all 3-point contacts', 'Average area of all 4-point contacts', 'Average area of all configurations', ...
    ...
    'Body size um'}); 
% xlswrite([foldername ExcelFileName '.xlsx'], {'label','speed','pace index','trot index','trip/tetra','freq','period','avr FP STD','avr perp M','sw v(AVR)','sw v(SD)','sw v(F)','sw v(M)','sw v(H)','sw s(AVR)','sw s(SD)','sw s(F)','sw s(M)','sw s(H)','sw t(AVR)','sw t(SD)','sw t(F)','sw t(M)','sw t(H)','period SD','stc t(F)','stc t(M)','stc t(H)','stc t(AVR)','stc t(SD)','varAEP_F','varAEP_M','varAEP_H','varPEP_F','varPEP_M','varPEP_H','stnc stabl','SD AEP','SD PEP','body size','F AEP','M AEP','H AEP','F PEP','M PEP','H PEP','F AEPy','M AEPy','H AEPy','F PEPy','M PEPy','H PEPy','F AEPySD','M AEPySD','H AEPySD','F PEPySD','M PEPySD','H PEPySD','F AEPx','M AEPx','H AEPx','F PEPx','M PEPx','H PEPx','F AEPxSD','M AEPxSD','H AEPxSD','F PEPxSD','M PEPxSD','H PEPxSD','PeriodSD1000','Speed SD','4feetON','framesON','body stbl','2+ pfON','F_AEP STD','M_AEP_AEP STD','H_AEP STD','F_PEP STD','M_PEP STD','H_PEP STD','(empty)','SwingV STD','F_deltaEP','M_deltaEP','H_deltaEP','non-can.','AVRcombTrace','STDcombTrace','AVR perpM','Max Mdist','swT+stT','duty Fact','Phase F','Phase M','Phase H','Phase LH vs LM','Phase LM vs LF','Phase RH vs RM','Phase RM vs RF','walk index','tTripod','tTransition','Stc Str F','Stc Str M','Stc Str H','Overall Stc Str','Overall Avg Disp','Overall Avg Path'},'1.Info_Sheet', 'A40');
...
Range = 'C41';
ActivesheetRange = get(sheet,'Range',Range);
set(ActivesheetRange, 'Value', {'=''10.Leg_combinations''!B86'});
% xlswrite([foldername ExcelFileName '.xlsx'], {'=''10.Leg_combinations''!B86'},'1.Info_Sheet', 'C41');

Range = 'D41';
ActivesheetRange = get(sheet,'Range',Range);
set(ActivesheetRange, 'Value', {'=''10.Leg_combinations''!B87'});
% xlswrite([foldername ExcelFileName '.xlsx'], {'=''10.Leg_combinations''!B87'},'1.Info_Sheet', 'D41');
...
Range = 'A13:W13';
ActivesheetRange = get(sheet,'Range',Range);
set(ActivesheetRange, 'Value', {'=SQRT(A15^2+B15^2)','=AVERAGE(A13,C13)','=SQRT(C15^2+D15^2)','','','','','','=SQRT(I15^2+J15^2)','=AVERAGE(I13,K13)','=SQRT(K15^2+L15^2)','','=SQRT(M15^2+N15^2)','=AVERAGE(M13,O13)','=SQRT(O15^2+P15^2)','','','','','','=SQRT(U15^2+V15^2)','=AVERAGE(U13,W13)','=SQRT(W15^2+X15^2)'});
% xlswrite([foldername ExcelFileName '.xlsx'], {'=SQRT(A15^2+B15^2)','=AVERAGE(A13,C13)','=SQRT(C15^2+D15^2)','','','','','','=SQRT(I15^2+J15^2)','=AVERAGE(I13,K13)','=SQRT(K15^2+L15^2)','','=SQRT(M15^2+N15^2)','=AVERAGE(M13,O13)','=SQRT(O15^2+P15^2)','','','','','','=SQRT(U15^2+V15^2)','=AVERAGE(U13,W13)','=SQRT(W15^2+X15^2)'},'16.AEP_PEP', 'A13');
