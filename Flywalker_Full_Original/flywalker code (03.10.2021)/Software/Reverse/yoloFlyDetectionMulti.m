function [positions] = yoloFlyDetectionMulti(srcDir, detector, rotation, maxAngDev, savefiles, curAngInit, uit, uitdata, uitindex)
...
if numel(srcStruc) < 1
    positions = [];
    uitdata{uitindex, 4} = "100%"; 
    uitdata{uitindex, 5} = "0 secs.";
    uitdata{uitindex, 6} = "No";
    uitdata{uitindex, 7} = "Folder does not have any images";
    set(uit, 'Data', uitdata);
    return
end
...
%     end
    ...
    for ci = 1:msh/2:h
        ...
        for cj = 1:msw/2:w
            ...
            cystart = ci; cystop = ci + msh - 1;
            cxstart = cj; cxstop = cj + msw - 1;
            I = Iraw(cystart:cystop, cxstart:cxstop, :);
            ...
            for ang = ang_aux          
                ...
                [bboxes,scores, labels] = detect(detector, Irot, 'Threshold', 0.33, 'ExecutionEnvironment','cpu');
                if length(scores) > 0
                ...
                if(maxScore > rotsMaxScore)
                    %disp(['y:' num2str(cystart) ' x:' num2str(cxstart) ' Score:' num2str(maxScore) ' Boxes found: ' num2str(size(bboxes))]);
    ...
    if(~isempty(rotsMaxBBox))
         ...
         pt_x = msw/2 + (x_bbc_lin - msw/2).*cosd(rotsMaxAngle) - (y_bbc_lin - msh/2).*sind(rotsMaxAngle);
         pt_y = msh/2 + (x_bbc_lin - msh/2).*sind(rotsMaxAngle) + (y_bbc_lin - msh/2).*cosd(rotsMaxAngle);
    ...
    uitdata{uitindex, 4} = string([num2str(round(100*k/numel(srcStruc))) '%']); 
    uitdata{uitindex, 5} = string([num2str(secToFinish) ' secs.']);
    set(uit, 'Data', uitdata);
    drawnow;
end
