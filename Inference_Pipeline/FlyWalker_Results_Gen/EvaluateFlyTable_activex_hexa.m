...
% Pre-collect fallback variables (computed before COM attempt, so available in catch)
fb_excel_file = [foldername ExcelFileName '.xlsx'];
fb_InfoData   = InfoData;
fb_body   = [TIME', Body_x', Body_y', Body_direction1', Body_direction2', Body_direction3', Body_orientation', Body_STDx', Body_STDy'];
fb_LFData = [TIME', DistanceLF', ParaDistLF', PerpDistLF', AngleLF'];
fb_LMData = [TIME', DistanceLM', ParaDistLM', PerpDistLM', AngleLM'];
fb_LBData = [TIME', DistanceLB', ParaDistLB', PerpDistLB', AngleLB'];
fb_RFData = [TIME', DistanceRF', ParaDistRF', PerpDistRF', AngleRF'];
fb_RMData = [TIME', DistanceRM', ParaDistRM', PerpDistRM', AngleRM'];
fb_RBData = [TIME', DistanceRB', ParaDistRB', PerpDistRB', AngleRB'];

% start Excel (try/catch: falls back to writecell if COM automation fails)
try
...
delete(exl);
    ...
    try
        if exist('exl', 'var')
            try; exlFile.Close(false); catch; end
            try; invoke(exl, 'Quit');   catch; end
    ...
    try
        ...
        disp(['writecell not available on this MATLAB version (' wb_err.message ')']);
        disp('Saving data as .mat backup (loadable in any MATLAB)...');
        try
            fb_mat_file = strrep(fb_excel_file, '.xlsx', '_data_backup.mat');
            save(fb_mat_file, 'fb_InfoData', 'fb_body', ...
                 'fb_LFData', 'fb_LMData', 'fb_LBData', ...
                 'fb_RFData', 'fb_RMData', 'fb_RBData');
            disp(['Data backup saved: ' fb_mat_file]);
        catch mat_err
            disp(['All output fallbacks failed: ' mat_err.message]);
        end
    end
end
